---
title: 好友链接
titleEn: Friends
description: 整理博客朋友、常用资源和项目入口。
descriptionEn: A curated list of friends, resources, and project links.
permalink: /friends/
friendsGroupTitle: 友情链接
feedPage:
  title: 好友动态
  titleEn: Friends Feed
  description: 通过 RSS 聚合好友博客的最新更新。
  descriptionEn: A live RSS-powered stream of updates from friendly blogs.
  path: /links/
  limit: 20
extraFeeds:
  - title: "1900"
    site: https://1900.live/
    feed: https://1900.live/rss.xml
    description: 只用于好友动态的订阅源。
    category: 额外动态
    enabled: true
    order: 100
  - title: Shephe
    site: https://www.shephe.com/
    feed: https://www.shephe.com/feed
    description: 只用于好友动态的订阅源。
    category: 额外动态
    enabled: true
    order: 101
  - title: 领先恐
    site: https://lingxiankong.github.io/
    feed: https://lingxiankong.github.io/pages/rss.xml
    description: 只用于好友动态的订阅源。
    category: 额外动态
    enabled: true
    order: 102
  - title: iMiku
    site: https://imiku.me/
    feed: https://imiku.me/index.xml
    description: 只用于好友动态的订阅源。
    category: 额外动态
    enabled: true
    order: 103
  - title: Eallion
    site: https://www.eallion.com/
    feed: https://www.eallion.com/index.xml
    description: 只用于好友动态的订阅源。
    category: 额外动态
    enabled: true
    order: 104
  - title: Frytea
    site: https://www.frytea.com/
    feed: https://www.frytea.com/atom.xml
    description: 只用于好友动态的订阅源。
    category: 额外动态
    enabled: true
    order: 105
  - title: Inks
    site: https://inkss.cn/
    feed: https://inkss.cn/atom.xml
    description: 只用于好友动态的订阅源。
    category: 额外动态
    enabled: true
    order: 106
  - title: LMS
    site: https://lms.pub/
    feed: https://lms.pub/feed
    description: 只用于好友动态的订阅源。
    category: 额外动态
    enabled: true
    order: 107
  - title: Lhasa
    site: https://lhasa.icu/
    feed: https://lhasa.icu/rss.xml
    description: 只用于好友动态的订阅源。
    category: 额外动态
    enabled: true
    order: 108
  - title: Hongtao
    site: https://hongtaoh.com/
    feed: https://hongtaoh.com/cn/blog/index.xml
    description: 只用于好友动态的订阅源。
    category: 额外动态
    enabled: true
    order: 109
groups:
  - title: 友情链接
    titleEn: Friends
    items:
      - title: 老孙博客
        titleEn: Old Sun's Blog
        description: 资深网民。
        descriptionEn: Replace this with your friends, projects, or favorite sites.
        href: https://www.imsun.org/
        feed: https://www.imsun.org/feed/
        avatar: https://img.imsun.org/avatar.jpg
        icon: fa-solid fa-link
      - title: 是非题
        description: 友邻博客。
        href: https://www.shifeiti.com
        feed: https://www.shifeiti.com/atom.xml
        icon: fa-solid fa-link
      - title: Shall We Code
        description: 一名后端工程师。
        href: https://waynerv.com/
        feed: https://waynerv.com/rss.xml
        icon: fa-solid fa-link
      - title: 杜老师说
        description: 杜老师说。
        href: https://dusays.com
        feed: https://dusays.com/atom.xml
        icon: fa-solid fa-link
      - title: 嗜血星空
        description: 建工学习者，分享部分个人收藏及笔记记录，网址及工具等。
        href: https://sxxkearth.github.io/
        icon: fa-solid fa-link
      - title: 木木木木木
        description: 友邻博客。
        href: https://immmmm.com
        feed: https://immmmm.com/atom.xml
        icon: fa-solid fa-link
      - title: McLsk888's Blog
        description: 友邻博客。
        href: https://mclsk888.top
        icon: fa-solid fa-link
      - title: 一大加贝
        description: 学习技术，热爱生活。
        href: https://tianheg.co/
        feed: https://tianheg.co/index.xml
        icon: fa-solid fa-link
      - title: 荷戟独彷徨
        description: 爱光学，爱生活，爱创造。
        href: https://guanqr.com/
        feed: https://guanqr.com/atom.xml
        icon: fa-solid fa-link
      - title: Amnesia's Blog
        description: 零碎的岛屿总会找到海。
        href: https://amnesia-f.vercel.app/
        icon: fa-solid fa-link
      - title: UyoAhz
        description: 集中精神，以气御剪。
        href: https://uyoahz.cn/
        feed: https://uyoahz.cn/atom.xml
        icon: fa-solid fa-link
      - title: 凡梦星尘空间站
        description: 再平凡的人也有属于他的梦想！
        href: https://lisenhui.cn
        feed: https://lisenhui.cn/rss.xml
        icon: fa-solid fa-link
      - title: Dvel's Blog
        description: Less is More.
        href: https://dvel.me/
        icon: fa-solid fa-link
      - title: Black Flies
        description: When nothing goes right, just go left.
        href: https://www.yyyzyyyz.cn/
        feed: https://www.yyyzyyyz.cn/atom.xml
        icon: fa-solid fa-link
      - title: 风记星辰
        description: 分享生活 · 留下感动。
        href: https://www.thyuu.com
        feed: https://www.thyuu.com/feed
        icon: fa-solid fa-link
      - title: 山岳库博
        description: 开发学习启发性二刺螈。
        href: https://kmar.top/
        icon: fa-solid fa-link
      - title: USTC-茶糜花开
        description: Science lights up the world.
        href: https://blog.starysky.top
        icon: fa-solid fa-link
      - title: 胡涂说
        description: 任抛星汉归园圃，留取乾坤盛酒囊。
        href: https://hutusi.com/
        feed: https://hutusi.com/feed.xml
        icon: fa-solid fa-link
      - title: 浮云翩迁之间
        description: 百代繁华一朝都，谁非过客；千秋明月吹角寒，花是主人。
        href: https://blognas.hwb0307.com
        icon: fa-solid fa-link
      - title: Echo's blog
        description: 韶华不为少年留 恨悠悠 几时休。
        href: https://www.liveout.cn/
        feed: https://www.liveout.cn/feed
        icon: fa-solid fa-link
      - title: Jdeal
        description: Life is like a Design.
        href: https://www.jdeal.cn
        feed: https://www.jdeal.cn/feed
        icon: fa-solid fa-link
      - title: 一蓑烟雨的博客
        description: 竹杖芒鞋轻胜马，谁怕？一蓑烟雨任平生。
        href: https://easyf12.top/
        feed: https://easyf12.top/atom.xml
        icon: fa-solid fa-link
      - title: 老卢
        description: 个人博客记录日常。
        href: https://blog.el9.cn/
        feed: https://blog.el9.cn/feed
        icon: fa-solid fa-link
      - title: 记忆一隅
        description: 凡事总需研究，才会明白。
        href: https://vlieo.com/
        feed: https://vlieo.com/atom.xml
        icon: fa-solid fa-link
      - title: This Cute World
        description: 脚下虽有万水千山，但行者必至。
        href: https://thiscute.world/
        icon: fa-solid fa-link
      - title: Leonus
        description: 进一寸有进一寸的欢喜。
        href: https://blog.leonus.cn
        feed: https://blog.leonus.cn/atom.xml
        icon: fa-solid fa-link
      - title: 二猫 blog
        description: 一个简单的 Blog 站点。
        href: https://jingxin18.cn
        feed: https://jingxin18.cn/feed
        icon: fa-solid fa-link
      - title: Dorad
        description: 地质、炼丹、编程、GIS、摄影。
        href: https://blog.cuger.cn
        feed: https://blog.cuger.cn/atom.xml
        icon: fa-solid fa-link
  - title: 推荐资源
    titleEn: Recommended
    items:
      - title: Astro
        titleEn: Astro
        description: 用于构建内容驱动网站的现代静态站点框架。
        descriptionEn: A modern static site framework for content-driven websites.
        href: https://astro.build/
        icon: fa-solid fa-rocket
      - title: Twikoo
        titleEn: Twikoo
        description: 轻量、可自部署的静态站点评论系统。
        descriptionEn: A lightweight, self-hostable comment system for static sites.
        href: https://twikoo.js.org/
        icon: fa-regular fa-comments
---

<div style="text-align:center;color:red;">暂不添加友链</div>

<h3>友链要求</h3>

- 不定时去朋友的博客溜达，如果网站打不开或是联系不上本人的话会单方面下架链接。
- 直接上来要求添加友链的一概不管，添加之前总要先熟悉或是评论几次吧，要不然感觉太突兀了
- 因为博主很喜欢一些漂亮的博客，当然优质的内容也是博客的精髓所在，所以如果你的博客真的很好看或是优质，博主会主动挂在朋友圈实时关注新的文章更新，待到时机成熟就会不要脸的请求添加友链
- 交换友链的网站需要有稳定的更新，目前本站不增添新站（6个月以内）
- 本站不添加资源型博客，近段时间不少资源型博客直接上来就想添加友链，无非就是为了引流，博主感到有些不快，故禁止此类博客。

<h3>我的友链</h3>

> 网站名称：长街短梦
>
> 网站地址：https://wangyunzi.com/
>
> 网站标志：https://blog.wangyunzi.com/avatar.png
>
> 站点截图：https://blog.wangyunzi.com/siteshot.png
> 
> 网站描述：此行山高路远，我只剩口袋玫瑰一片


<div style="text-align:center;"><span class="with-love" id="animate1">
    <i class="icon-heart"></i>
  </span>请谨慎是否添加博主友链，博主希望友链是平时能够相互互动的，而不是添加了躺在我的友链里面，然后迁移博客的时候看着一堆自己不熟悉的链接<span class="with-love" id="animate2">
    <i class="icon-heart"></i>
  </span></div>
